<?php
	...
	if (empty($_POST['coachParent'])){ // je ne met pas de 'isset' car c'est un select, et il est 'defini' vide de base, il ne peut donc pas ne pas être défini
		$messageErreur.="<br>- Coach parent non choisi ";
		$erreur=true;
	}
	...
	if ($erreur) { // si la variable d'erreur est vrai, donc s'il y a une erreur
		...
		while ($data = $result->fetch()) {
			if ($data["AdresseMailProfil"] == $email) {
				$erreur = true;
				$messageErreur .= "</br>- L'adresse mail entrée est déjà utilisée !";
			} 
		}
		...
		if($erreur) { 
			...
			INSERT INTO joueur (DateDeNaissance, Sexe, Nom, Prenom, Adresse, RefCoach)
			VALUES ('$date', '$sexe', '$nom', '$prenom', '$adresse', '$IdCoach')

			";

			$BDD->query($insertJoueur);


			$query2 = "

			INSERT INTO profil (AdresseMailProfil, MotDePasseProfil, RefCoach, RefJoueur) 
			VALUES ('$email', '$mdpCrypte', 0, (SELECT MAX(IdJoueur) FROM Joueur))

			"; // je prépare la requête d'insertion pour créer le compte
