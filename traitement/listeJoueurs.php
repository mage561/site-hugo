<?php
	...
	$refPersonne = $_SESSION["refPersonne"];

	$query = "SELECT * FROM joueur WHERE RefCoach = $refPersonne";
	$result = $BDD->query($query);

	while ($data = $result->fetch()) {
		echo "<p>".$data["Nom"]." ".$data["Prenom"]."</p>";  // on affiche la liste des joueurs lignes par lignes (grâce au while)
	}
